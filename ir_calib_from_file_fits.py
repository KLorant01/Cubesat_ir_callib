...
DIR_name: str = f"{datetime.now().strftime("%Y_%m_%d_%H_%M_%S")}_fits"
os.mkdir(f"Corrected/{DIR_name}")

eeprom_data_list = []


# ======== INIT ========
for num_inc in range(SENSOR_NUM):
    ir_num = num_inc + 20
    ...
    eeprom_data_list.append(eeprom_data)





for block_num in range(SET_SIZE):
    for num_inc in range(SENSOR_NUM):
        ir_num =  num_inc + 20
        img = fits.getdata(f"1329/raw_block-n{block_num:03d}-{ir_num}.fits")
        eeprom_data = eeprom_data_list[num_inc]

        tic = time()

        Vdd25 = read_eeprom(eeprom_data, '26')  # 0x2426
        tracker_array.append(Vdd25)
        ...
        filename = f"Corrected/{DIR_name}/corr-n{block_num:03d}-{ir_num}.fits"
        fits.writeto(filename, To_image, overwrite=True)
        Delata_act = time() - tic
        time_tracker += Delata_act
        time_counter += 1
        print(f"{block_num}-{ir_num} \t Exec time: {Delata_act}")

print(f"\noverall time: {time_tracker}")
print(f"avg time: {time_tracker / time_counter}")

print(tracker_array)
plt.plot(tracker_array, range(len(tracker_array)))
plt.show()